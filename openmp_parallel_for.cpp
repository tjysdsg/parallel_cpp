...
#pragma omp parallel num_threads(3) default(none)                              \
    shared(team, index) /// team is shared across 3 threads
  {
    /// increment-and-get, this is atomic
    /// https://en.cppreference.com/w/cpp/atomic/atomic/operator_arith
    int t = ++team;

#pragma omp for lastprivate(index) /// the latest value of index is kept as the
                                   /// value of index in the outer scope
    for (int i = 0; i < 18; ++i) {
      index = i;
